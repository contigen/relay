...
export default function ThreadRow({ thread }: ThreadRowProps) {
  ...
  const messages = (useQuery(api.threads.getMessagesByThread, {
    threadId: thread._id,
  }) ?? []) as MessageDoc[]
  const badge = getBadgeStyle(thread.status)

  const formattedDate = new Date(thread.lastActivity).toLocaleDateString(
    'en-US',
    {
      month: 'short',
      day: 'numeric',
              ...
              >
                ...
                >
                  <div className='flex items-center justify-between gap-4 mb-1.5 opacity-60 text-[10px]'>
                    <span>
                      {isOut
                        ? `Relay → ${thread.vendorName}`
                        : `${thread.vendorName} → Relay`}
                    </span>
                    <span>
