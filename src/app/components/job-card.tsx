...
export default function JobCard({
  ...
  const getStatusBadge = () => {
    switch (job.status) {
      case "completed":
        return { text: "COMPLETED", color: "text-[#16a34a]", bg: "bg-[#ecfdf5]", dot: "bg-[#16a34a]" };
      case "needs_decision":
        return { text: "INPUT REQ", color: "text-[#d97706]", bg: "bg-[#fffbeb]", dot: "bg-[#d97706]" };
      case "failed":
        return { text: "FAILED", color: "text-[#dc2626]", bg: "bg-[#fef2f2]", dot: "bg-[#dc2626]" };
      default:
        return { text: "ACTIVE", color: "text-[#16a34a]", bg: "bg-[#ecfdf5]", dot: "bg-[#16a34a]" };
    }
  };
  ...
  return (
    <div
      ...
      className={`border transition-all cursor-pointer p-5 bg-white mb-4 ${
        selected ? "border-[#0a0a0a] shadow-sm" : "border-[#e5e5e5] hover:border-[#a3a3a3]"
      }`}
    >
      ...
      <div className="space-y-1.5 font-mono text-[11px] text-[#525252] border-t border-[#f0f0f0] pt-3">
        <div className="flex justify-between">
          ...
          <span className="text-[#0a0a0a] font-medium">{job.parsedIntent?.category || "Services"}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#8a8a8a]">Location:</span>
          <span className="text-[#0a0a0a]">{job.parsedIntent?.location || "Remote / Area"}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#8a8a8a]">Budget:</span>
          <span className="text-[#0a0a0a]">{job.parsedIntent?.budget || "Flexible"}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#8a8a8a]">Created:</span>
          <span className="text-[#737373]">{elapsed < 1 ? "just now" : `${elapsed}m ago`}</span>
        </div>
      </div>
