...
export default function TerminalBox({
  ...
  return (
    <div className="border border-[#e5e5e5] bg-white p-6 mb-6">
      ...
      <div className="space-y-2.5 font-mono text-xs mb-6">
        {steps.map((step, idx) => (
          <div key={idx} className="flex items-start gap-2.5">
            ...
            {step.status === "running" && (
              <span className="text-[#2563eb] font-bold select-none animate-spin">●</span>
            )}
            {step.status === "pending" && (
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2 pt-2 border-t border-[#f0f0f0]">
        <input
          type="text"
          value={prompt}
