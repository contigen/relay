...
export default function DashboardView({ initialTab = "dashboard" }: DashboardViewProps) {
  const [activeTab, setActiveTab] = useState<"dashboard" | "setup" | "history" | "verify">(initialTab);
  ...
  const activeJob = jobs.find((j) => (selectedJobId ? j._id === selectedJobId : true)) ?? jobs[0];
  ...
  const bestQuote = threads.find((t) => t.quote)?.quote ?? (quotesCount > 0 ? `${quotesCount} quotes` : "—");

  const terminalSteps = [
    {
      text: activeJob ? `Task parsed: ${activeJob.parsedIntent?.category || "Sourcing"} in ${activeJob.parsedIntent?.location || "target area"}` : "Standby for new sourcing task...",
      status: activeJob ? ("done" as const) : ("pending" as const),
    },
    {
      text: activeJob?.firecrawlResults?.length ? `Firecrawl verified ${activeJob.firecrawlResults.length} vendor candidates` : "Searching verified vendor registry via Firecrawl...",
      status: (activeJob?.firecrawlResults?.length ? "done" : activeJob?.status === "researching" ? "running" : "pending") as "done" | "running" | "pending",
    },
    {
      text: threads.length > 0 ? `AgentMail coordinated ${threads.length} vendor inquiry threads` : "Provisioning dedicated AgentMail inboxes...",
      status: (threads.length > 0 ? "done" : activeJob?.status === "outreaching" ? "running" : "pending") as "done" | "running" | "pending",
    },
    {
      text: activeJob?.status === "completed" ? "Final executive report generated and dispatched to client" : activeJob?.status === "needs_decision" ? "Awaiting human-in-the-loop decision" : "Listening for vendor quote replies on webhook...",
      status: (activeJob?.status === "completed" ? "done" : "running") as "done" | "running",
    },
  ];
    ...
    <div className="min-h-screen bg-[#fafafa] text-[#0a0a0a] flex flex-col justify-between font-mono">
        ...
        <header className="border-b border-[#e5e5e5] bg-white sticky top-0 z-40">
          <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
            ...
            <nav className="flex items-center gap-8">
              {(["setup", "dashboard", "history", "verify"] as const).map((tab) => (
                <button
                  key={tab}
          ...
          {activeTab === "setup" ? (
            ...
            <div className="space-y-4">
              ...
              <div className="border border-[#e5e5e5] bg-white p-6">
                {jobs.map((j) => (
                  ...
                  >
                    <div>
                      <h4 className="font-serif text-lg">{j.parsedIntent?.description || j.rawTask}</h4>
                      <p className="text-xs text-[#737373] font-mono mt-1">{j.userEmail} · {new Date(j.createdAt).toLocaleDateString()}</p>
                    </div>
                    <span className="font-mono text-xs text-[#16a34a] uppercase">{j.status}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border border-[#e5e5e5] bg-white p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 border border-[#e5e5e5] bg-[#fafafa]">
                  <span className="text-xs uppercase text-[#737373] block mb-1">CONVEX HOSTING</span>
                  <span className="text-sm font-semibold text-[#16a34a] block">PASS (200 OK)</span>
                  <span className="text-[11px] text-[#737373] mt-2 block">energetic-koala-352.convex.site</span>
                </div>
                <div className="p-4 border border-[#e5e5e5] bg-[#fafafa]">
                  <span className="text-xs uppercase text-[#737373] block mb-1">AI ENGINE</span>
                  <span className="text-sm font-semibold text-[#16a34a] block">GEMINI 3.6 FLASH</span>
                  <span className="text-[11px] text-[#737373] mt-2 block">Vercel AI SDK Google Adapter</span>
                </div>
                <div className="p-4 border border-[#e5e5e5] bg-[#fafafa]">
                  <span className="text-xs uppercase text-[#737373] block mb-1">AGENTMAIL WEBHOOK</span>
                  <span className="text-sm font-semibold text-[#16a34a] block">ONLINE</span>
                  <span className="text-[11px] text-[#737373] mt-2 block">/webhook/agentmail</span>
                </div>
              </div>
            </div>
          ) : (
            <>
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 mb-8 border-b border-[#e5e5e5]">
                <div>
                  ...
                  <p className="font-mono text-xs text-[#737373] mt-2">
                    Client: <span className="text-[#0a0a0a]">{activeJob?.userEmail || "user@relay.to"}</span> · Inbox: <span className="text-[#0a0a0a]">{activeJob?.agentEmail || "relay-agent@agentmail.to"}</span>
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <MetricCard label="QUOTES SECURED" value={bestQuote} highlight={quotesCount > 0} />
                <MetricCard label="VENDORS CONTACTED" value={threads.length || 3} />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                      ...
                      <button
                        onClick={() => {
                          if (activeJob) {
                            startPipeline({ userEmail: activeJob.userEmail, rawTask: activeJob.rawTask });
                          }
                        }}
                        className="p-2.5 border border-[#e5e5e5] bg-white hover:border-[#0a0a0a] text-center text-[#525252] hover:text-[#0a0a0a] transition-colors"
                      >
                  ...
                  <div className="pt-4 border-t border-[#e5e5e5] flex items-center justify-between text-xs font-mono">
                    <span className="text-[#8a8a8a] uppercase text-[10px]">WORKFLOW STATUS</span>
                ...
                <div className="lg:col-span-8 space-y-6">
                  ...
                  <TerminalBox
                    steps={terminalSteps}
                    statusLabel={activeJob ? activeJob.status.toUpperCase() : "LIVE"}
                    onSendQuery={(q) => {
                      if (activeJob) {
                        startPipeline({ userEmail: activeJob.userEmail, rawTask: q });
                      }
                    }}
                  />
                  ...
                  <div className="border border-[#e5e5e5] bg-white p-6">
                    ...
                    {threads.length === 0 ? (
                      <div className="py-12 text-center text-xs font-mono text-[#8a8a8a]">
                        No active vendor threads yet. Click &quot;New Sourcing Run&quot; to begin.
                      </div>
                    ) : (
                      threads.map((thread) => (
                        <ThreadRow
                          ...
                          thread={thread}
                          jobDescription={activeJob?.parsedIntent?.description ?? activeJob?.rawTask}
                          agentInboxId={activeJob?.agentInboxId || thread.agentInboxId}
                          agentEmail={activeJob?.agentEmail || thread.agentEmail}
                        />
                      ))
                    )}
                  </div>
      ...
      <footer className="border-t border-[#e5e5e5] bg-white py-6 mt-12">
        <div className="max-w-7xl mx-auto px-6 text-center text-[10px] font-mono uppercase tracking-widest text-[#8a8a8a]">
          ALL SOURCING ACTIONS AUTONOMOUSLY RECORDED ON CONVEX · POWERED BY GEMINI 3.6 FLASH · FIRECRAWL · AGENTMAIL
        </div>
      </footer>
